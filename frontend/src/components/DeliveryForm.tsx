...
export default function DeliveryForm({ delivery, onSubmit }) {
  const { register, handleSubmit } = useForm({ defaultValues: delivery });

  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
      <div className="flex flex-col gap-1">
        <label htmlFor="customer" className="text-sm font-medium text-secondary-200">
          Customer
        </label>
        <input
          id="customer"
